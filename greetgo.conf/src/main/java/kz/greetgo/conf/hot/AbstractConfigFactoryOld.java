...
import static kz.greetgo.conf.hot.AbstractConfigFactoryUtil.strToBool;

public abstract class AbstractConfigFactoryOld {
  private final List<HotConfigMediator> mediators = synchronizedList(new ArrayList<HotConfigMediator>());
  ...
  /**
   * Returns appending stream for storage of config
   *
  ...
  public void reset() {
    ...
    for (HotConfigMediator mediator : mediators) {
      mediator.loaded.set(false);
    }
  }
  ...
  public <T> T createConfig(Class<T> configInterface) {
    return (T) Proxy.newProxyInstance(getClass().getClassLoader(), new Class<?>[]{configInterface},
        getMediator(configInterface));
  }

  private <T> HotConfigMediator getMediator(Class<T> class1) {
    for (HotConfigMediator inv : mediators) {
      if (inv.class1.equals(class1)) return inv;
    }
    {
      HotConfigMediator mediator = new HotConfigMediator(class1);
  ...
  private class HotConfigMediator implements InvocationHandler {
    final Class<?> class1;
    final AtomicBoolean loaded = new AtomicBoolean(false);
    private final Map<String, Object> data = new ConcurrentHashMap<>();

    HotConfigMediator(Class<?> class1) {
    ...
    void printParameterIni(PrintWriter pr, Method method) throws Exception {
      ...
      if (description != null) {
        pr.println("##");
        for (String docStr : description.value().split("\\n")) {
          pr.println("## " + docStr);
        }
        pr.println("##");
      }
